...
Scenario: Creacion
  ...
  Then I send a signal to user 3 containing "Done2"
  Then I wait
...
Scenario: Editar post
  ...
  Then I edit the existing post with title "$$number4" to have the new number "$number5" and new content "Esto fue actualizado por una prueba automatizada" on "<Host>"
  Then I delete the post with number "$$number5" on "<Host>"
  

@user5 @web
Scenario: Despublicar post
  Given I login to the Ghost application with username "<LoginUsername>" and password "<LoginPassword>" on "<Host>"
  When I create a new post number "$number5" and content "Esto es una prueba automatizada" on "<Host>"
  Then I wait
  Then I verify the post was created with number "$$number5" on "<Host>"
  Then I unpublish the post with number "$$number5" on "<Host>"
  Then I wait
  Then I check the post with number "$$number5" is not published on "<Host>"
  Then I delete the post with number "$$number5" on "<Host>"
